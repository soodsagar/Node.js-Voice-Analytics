...
exports.voiceAnalyticsAverage = function( app, routes, http, path, crypto, applogger, logger, sanitize ) {
    console.log('loading user voiceAnalyticsAverage function');

    app.post('/apiv1/voiceAnalyticsAverage', function(req, response) {
            ...
            if(1 == valid_credentials){
                connection.query('SELECT appname FROM APPLICATION_KEYS WHERE appkey=' +
                    ...
                    if(match_rows[0] != undefined){
                        ...
                        // ===================================================
                        var sqlQueryCommand = "SELECT id, time_created, command as voice_command, conkey, homeprofileID FROM `_MOBILE_SAAS`.`VoiceAnalytics` ORDER BY time_created";
                            ...
                            if(match_rows_QC[0] != undefined){
                                ...
                                if ( !doneFlag ){
                                    ...
                                    while ( arrCount < arrayLen ){
                                        ...
                                        var homeid = match_rows_QC[arrCount].homeprofileID;
                                        time.push(v_command);
